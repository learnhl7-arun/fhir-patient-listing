<script lang="ts">
  ...
  import { fetchDoctorsRaw, doctorsStoreRaw } from '../stores/doctorStore';
  ...
  let doctorList = [];
  $: doctorsStoreRaw.subscribe(value => {
    doctorList = value.map(resource => ({
    id: resource.id || '-',
    identifierNPI: resource.identifier ? resource.identifier.find(id => id.type.coding[0].code === "NPI")?.value : '',
    identifierSS: resource.identifier ? resource.identifier.find(id => id.type.coding[0].code === "SS")?.value : '',
    firstName: resource.firstName || '',
    lastName: resource.lastName || '',
    gender: resource.gender || 'male',
    qualificationCode: resource.qualificationCode || '',
    qualificationDisplay: resource.qualificationDisplay || ''
  }));
  });
  ...
  function openModal(doctor = {}, isEdit = false, isAdd = false) {
    ...
    editMode = isEdit;
    addMode = isAdd;
    selectedDoctor = isEdit ? { ...doctor } : { id: "", identifierNPI: "", identifierSS: "", firstName: "", lastName: "", gender: "male", qualificationCode: "" };
  }
    ...
    <tbody class="bg-white divide-y divide-gray-200 overflow-y-auto">
      {#each doctorList as doctor}
        <tr>
          ...
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{doctor.firstName}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{doctor.lastName}</td>
          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{doctor.qualificationDisplay}</td>
        ...
        {#if addMode || editMode}
          <select class="mt-1 p-2 w-full border border-gray-300 rounded" bind:value={selectedDoctor.qualificationCode} required>
            {#each qualifications as qual}
              <option value={qual.code}>{qual.display}</option>
            {/each}
          </select>
        {:else}
          <span class="mt-1 p-2 w-full border border-gray-300 rounded block">{qualifications.find(q => q.code === selectedDoctor.qualificationCode)?.display}</span>
        {/if}
      </div>
    </div>
